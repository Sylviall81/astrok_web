...
export default function BookingPage() {
  ...
  const calLink = calParam
    ? decodeURIComponent(calParam)
    : "kaleidoscope-astrologia-psicologica-tj7xzl"

  useEffect(() => {
    const embedEl = document.getElementById("cal-embed")
    if (embedEl) embedEl.innerHTML = ""

    const win = window as any

    // Si Cal ya está cargado, solo borramos el namespace para forzar reinicio limpio.
    // Borrar window.Cal entero y recargar el script causa una carrera asíncrona que deja el embed en blanco.
    if (win.Cal?.ns?.kaleidoscope) {
      delete win.Cal.ns.kaleidoscope
    }

    const timer = setTimeout(() => {
      if (!win.Cal) {
        // Primera carga: inicializa Cal y carga el script
      ...
      Cal.ns.kaleidoscope("on", {
        action: "bookingCancelled",
        callback: () => router.push("/servicios"),
      })
    }, 100)
    ...
    <section className="py-16 md:py-24">
      <div className="container-custom">
        <div className="max-w-4xl mx-auto text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-lato font-bold text-primary mb-6">
            Agenda tu sesión
          </h1>
        </div>
